...
exports.uncoverOriginalNumberArray = exports.adjustNumberArray = void 0;
 ...
 */
function adjustNumberArray(array) {
    if (array.length === 0)
        return;
...
exports.adjustNumberArray = adjustNumberArray;
/**
 * Modifies the array into the array it was before `adjustNumberArray` was called on it.
 */
function uncoverOriginalNumberArray(adjustedArray) {
    if (adjustedArray.length === 0)
        return;
    const paddingNeeded = adjustedArray[adjustedArray.length - 4];
    if (![0, 1, 2, 3].includes(paddingNeeded))
        throw new Error('Unexpected padding.');
    for (let i = 0; i < paddingNeeded + 4; i++)
        adjustedArray.pop();
}
exports.uncoverOriginalNumberArray = uncoverOriginalNumberArray;
