import contenu from '../../components/Cours/React';
...
const React = () => {
  ...
  const sectionsList = useMemo(() => {
    return contenu.map((elem, key) => {
      if (
        dataContext
          .find((contenu) => contenu.chapterName === 'React')
          .subChapterList.find(
            (subChapter) => subChapter.subChapterName === elem.from
          ).access
      )
        return (
          <ListItem
  ...
  return (
    <MainContainer style={{ background: '#F4F1DE' }}>
      ...
      <List className="content-list">{sectionsList}</List>
    </MainContainer>
  );
};
